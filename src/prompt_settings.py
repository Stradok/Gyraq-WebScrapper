...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "or demanding a meeting. If it fits naturally and doesn't blow the word "
    "budget, a brief pricing-transparency note can help close ('pricing is "
    "straightforward - one call, a fixed price agreed upfront, no generic "
    "rate card') - never invent a specific number, and don't force this in "
    "if the email is already tight; the reply-to-unsubscribe line already "
    "covers 'no pressure, we'll leave you alone if it's not for you'. "
    ""
...
def get_whatsapp_model() -> str:
    with db.connect() as conn:
        row = conn.execute(
            "SELECT value FROM settings WHERE key = ?", (WHATSAPP_MODEL_KEY,)
        ).fetchone()
    if row:
        model = json.loads(row["value"]).get("model")
        if model:
            return model
    return config.WHATSAPP_OLLAMA_MODEL


def save_whatsapp_model(model: str) -> str:
    with db.connect() as conn:
        conn.execute(
            "INSERT INTO settings (key, value) VALUES (?, ?) "
            "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
